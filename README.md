...
- Bulk draft simulator capped at 250 runs with strategy comparison, controlled variation, first-five-round build optimization, and zip export for training/control review
...
- Visible draft simulation overlay for auto-pick, sim-to-next-pick, and sim-rest-of-draft actions
...
- Weekly Positional Edge strategy that hunts for starters who create a projected weekly advantage over the league baseline at their position
- Source-aware ranking database with CSV, TSV, XLSX/XLSM, and JSON import
- Broader ranking import support for table-style exports such as 4for4 `Position-Rank`, `FF Pts`, `ADP ( Average )`, workbook title rows, tier sections, multi-sheet XLSX files, and market-score columns
- Rankings tab cheat sheet with searchable, filterable, source-aware player rows for rank, ADP, projection, tier, bye, sources, and role notes
...
- Player-name detail popups in the draft room with overview, imported news/context, depth chart, schedule/bye timing, and an explicit draft action
...
- Recommendation cards explain why this player, why now, why over alternatives, and why over other positions at the current draft slot
...
Use `ranking-import-template.csv` as the easiest format, or upload a `.xlsx` workbook with a player table on any sheet.

The importer scans CSV/TSV and workbook sheets for the real player-table header instead of assuming row 1. It recognizes common variations for player/name, position/POS, team/TM, rank/RK/ECR, tier, ADP/AVG, projection/points, and market score columns, then skips non-player title rows or section headers.
...
Use the Sleeper connector in the League tab to find a Sleeper league by username or user ID and season. Importing updates league settings, assigns team names from the imported Sleeper rosters, matches the user's Sleeper account to `My team`, applies Sleeper's current draft slots plus traded-pick records to the Pick Order tab, and saves last season's rostered players plus draft rounds as keeper-option context.

Sleeper traded picks are pulled from both the league traded-picks endpoint and the selected draft's traded-picks endpoint. Draft-level records are treated as authoritative for that draft, then applied against the original draft slot for that pick and updated to the current Sleeper owner. This keeps re-imports accurate when a pick has been traded recently or traded more than once, and the import status shows how many returned traded-pick records were applied to Pick Order.
...
Use the Bulk Simulator tab to run up to 250 mock drafts at once. You can compare all strategies or focus on one strategy. The simulator uses small batched runs so the browser stays responsive.
...
Large batches and Weekly Positional Edge single-strategy sessions yield between runs so the browser stays responsive while the progress bar advances.

After a simulation batch finishes, use Export zip to download the full batch for Google Drive or formula tuning. The zip includes a manifest, summary JSON, strategy/build CSVs, one JSON file per simulation run, all pick rows, user-pick availability snapshots, and a JSONL training file.
...
Use the Draft mode selector in the setup sidebar to switch from `Mock simulation` to `Live draft entry`. In live mode, the app does not auto-simulate the room. Use the explicit `Draft` button in the Available Players list, Best Pick panel, tiers, or player detail popup to record the actual player selected for whichever team is currently on the clock. Clicking a player name opens the detail popup instead of drafting them.
...
Mock mode includes `Sim rest of draft`, which auto-selects future picks for every team from the current board state. Auto-pick and simulation controls show a popup with recent player selections while the room advances.
...
The Draft Awards sub-tab works for completed mock drafts and live draft entries. It names the winner plus the next three for Best Draft Overall, Best Pick, Worst Pick, Most Likely Champion, and Most Likely Last Place using draft value, projected lineup strength, roster balance, and 500 simulated seasons with regular-season schedule variance plus a playoff bracket.
