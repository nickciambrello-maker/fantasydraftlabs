...
function bestLineupForBulk(roster) {
  ...
  const byPosition = (position) => projected
    .filter((player) => player.position === position)
    .sort((a, b) => b.weeklyProjection - a.weeklyProjection);
  const lineup = [];
  lineup.push(...byPosition("QB").slice(0, Number(LEAGUE.roster.QB || 0)));
  lineup.push(...byPosition("RB").slice(0, Number(LEAGUE.roster.RB || 0)));
  lineup.push(...byPosition("WR").slice(0, Number(LEAGUE.roster.WR || 0)));
  lineup.push(...byPosition("TE").slice(0, Number(LEAGUE.roster.TE || 0)));
  const used = new Set(lineup.map((player) => player.id));
  lineup.push(...projected
    .filter((player) => ["RB", "WR", "TE"].includes(player.position) && !used.has(player.id))
    .sort((a, b) => b.weeklyProjection - a.weeklyProjection)
    .slice(0, Number(LEAGUE.roster.FLEX || 0)));
  lineup.push(...byPosition("K").slice(0, Number(LEAGUE.roster.K || 0)));
  lineup.push(...byPosition("DEF").slice(0, Number(LEAGUE.roster.DEF || 0)));
...
renderRecommendations = function renderRecommendationsV150() {
  const viewedDraft = state.completedDrafts.find((draft) => draft.id === state.viewedDraftId);
  const total = LEAGUE.teams * LEAGUE.rounds;
  if (viewedDraft) {
    renderCurrentPickHeader("Read-only review");
