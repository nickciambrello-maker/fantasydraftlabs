...
const SIMULATOR_ENGINE_VERSION = "league-decision-lab-v2";
...
function seasonSimulationWorker() {
  ...
  if (typeof Worker === "undefined" || window.location.protocol === "file:") return null;
  try {
    BULK_SEASON_WORKER = new Worker("./simulation-worker.js?v=111");
...
function simulateBulkDraft(strategy, runIndex, modelKey = bulkSimulationModelKey()) {
  ...
  try {
    ...
    return {
      ...
      userRoster: detailedUserAnalysis.roster.map((player) => ({ ...player, weeklyProjection: projectionForPlayer(player) })),
...
function projectionCoverageForRuns(runs) {
  ...
  return players.filter((player) => projectionProfileForPlayer(player).projectionType === "imported").length / players.length;
}
...
function deriveSimulationSurvival(runs) {
  ...
  runs.forEach((run) => {
    ...
    const draftedAt = new Map((run.allPicks || []).map((pick) => [pick.player.id, pick.pick]));
...
function buildDraftPlanPriority(summary, survivalRows) {
  ...
  const roster = rosterFor(state.userTeam), counts = positionCounts(roster);
  const rows = PLAYERS.map((player) => {
    const labValue = clampNumber(1 - ((player.consensusRank || PLAYERS.length) - 1) / Math.max(1, PLAYERS.length - 1), 0, 1);
    const evidence = survivalRows.find((row) => row.type === "player" && row.round === currentRound && row.id === player.id);
    const tierEvidence = survivalRows.find((row) => row.type === "tier" && row.round === currentRound && row.id === `${player.position}-${player.tier || "unknown"}`);
    ...
    const replacementCost = playerReplacementCost(player);
    ...
    const redundancyPenalty = recommendationRosterPenalty(player, roster, state.currentPick, bestStrategyId) >= 100 ? 20 : recommendationRosterPenalty(player, roster, state.currentPick, bestStrategyId) >= 45 ? 9 : 0;
    ...
    return {
      ...
      snipeThreats: scoutingSnipeEvidence(player, state.userTeam, state.currentPick).threats.slice(0, 3).map((threat) => activeTeamName(threat.team)),
      replacementCost,
...
function summarizeBulkResults(runs) {
  ...
  const summary = {
    ...
    comparison,
    confidence,
    outcomeBasis: runs.some((run) => run.seasonSimulationCount) ? "Actual playoff qualification counts from simulated seasons" : "Relative Draft Strength fallback",
...
function updateBulkProgress() {
  ...
  if ($("bulkScheduledSummary")) {
    ...
    const workerLabel = typeof Worker !== "undefined" && window.location.protocol !== "file:" ? "background season worker" : "main-thread fallback";
    $("bulkScheduledSummary").textContent = `${state.bulk.count} run${state.bulk.count === 1 ? "" : "s"} per ${state.bulk.mode === "compare" ? "strategy" : "selected strategy"} · ${strategies} strateg${strategies === 1 ? "y" : "ies"} · ${total} total · ${bulkSeasonSimulationCount()} season samples/run · ${workerLabel}`;
  }
...
function finishBulkSimulationBatch(runs, modelKey, { cancelled = false } = {}) {
  ...
  const summary = summarizeBulkResults(runs);
...
function startBulkSimulations() {
  ...
  const runBatch = async () => {
    if (state.bulk.cancelRequested) {
      finishBulkSimulationBatch(runs, modelKey, { cancelled: true });
      return;
    }
    try {
      const batchSize = state.bulk.depth === "deep" || schedule.length >= 200 ? 1 : state.bulk.depth === "quick" ? 4 : 2;
      ...
      finishBulkSimulationBatch(runs, modelKey);
    } catch (error) {
      state.bulk.running = false;
      state.bulk.error = `Simulation failed: ${error?.message || "Unknown error"}. Draft state was restored.`;
      state.bulk.cancelRequested = false;
      if (runs.length) finishBulkSimulationBatch(runs, modelKey, { cancelled: true });
      else renderBulkSimulator();
    }
  };
...
function renderBulkSimulator() {
  ...
  if (state.bulk.running && !data?.summary) {
    $("bulkResults").innerHTML = `<div class="bulk-empty sim-running"><h3>Building the Draft Plan</h3><p>Running completed drafts in browser-safe batches. Progress is shown above and cancellation is available between simulations.</p><div class="sim-skeleton"></div></div>`;
    return;
  }
  if (!data?.summary) {
    ...
    $("bulkResults").innerHTML = `<div class="bulk-empty"><h3>No simulations completed</h3><p>${escapeHtml(message)}</p><p class="helper">Compare mode treats the count as runs per strategy. Standard schedules 25 runs for each of ${BULK_STRATEGIES.length} strategies.</p></div>`;
    return;
  }
...
function predictedKeeperSelections(userTeam = null, userSet = null) {
  return Array.from({ length: LEAGUE.teams }, (_, index) => {
    ...
    let entries = actual;
    if (team === userTeam && userSet) entries = userSet.players.map((row) => ({ playerId: row.player.id, round: row.round, yearsKept: row.sourceCandidate?.yearsKept || 0, salary: row.salary, acquisitionType: row.acquisitionType || row.sourceCandidate?.acquisitionType, franchise: Boolean(row.franchise || row.sourceCandidate?.franchise) }));
    else if (!actual.length) {
      const top = keeperSetOptimizer(team)[0];
      entries = top ? top.players.map((row) => ({ playerId: row.player.id, round: row.round, yearsKept: row.sourceCandidate?.yearsKept || 0, salary: row.salary, acquisitionType: row.acquisitionType || row.sourceCandidate?.acquisitionType, franchise: Boolean(row.franchise || row.sourceCandidate?.franchise) })) : [];
    }
...
function applyKeeperSet(team, setId) {
  ...
  if (!set) return;
  setKeeperEntriesForTeam(team, set.players.map((row) => ({ playerId: row.player.id, round: row.round, yearsKept: row.sourceCandidate?.yearsKept || 0, salary: row.salary, acquisitionType: row.acquisitionType || row.sourceCandidate?.acquisitionType, franchise: Boolean(row.franchise || row.sourceCandidate?.franchise) })));
  saveKeeperSelections();
  refreshKeeperPicksInCurrentDraft();
...
renderKeeperEditor = function renderKeeperEditorV13(selections = state.keeperSelections, league = LEAGUE, names = state.teamNames) {
  const editor = $("keeperEditor");
  if (!editor) return;
  const maxKeepers = Math.max(1, normalizeKeeperRules(league.keeperRules).maxKeepers || 1);
  ...
  editor.innerHTML = Array.from({ length: league.teams }, (_, index) => {
    ...
    const rows = Array.from({ length: maxKeepers }, (_, slot) => {
      ...
      const roundOptions = [`<option value="">No keeper</option>`, ...Array.from({ length: league.rounds }, (__, roundIndex) => `<option value="${roundIndex + 1}" ${Number(entry.round) === roundIndex + 1 ? "selected" : ""}>Round ${roundIndex + 1}</option>`)].join("");
      const acquisitionOptions = [["drafted", "Drafted"], ["waiver", "Waiver / undrafted"], ["trade", "Acquired by trade"]].map(([value, label]) => `<option value="${value}" ${entry.acquisitionType === value ? "selected" : ""}>${label}</option>`).join("");
      const franchiseDisabled = normalizeKeeperRules(league.keeperRules).franchiseExemptions ? "" : "disabled";
      return `<div class="keeper-rule-row"><label>Keeper ${slot + 1}<input data-keeper-v13-player="${team}:${slot}" list="keeperPlayerOptions" value="${escapeHtml(player?.name || "")}" placeholder="Player name" /></label><label>Acquisition<select data-keeper-v13-acquisition="${team}:${slot}">${acquisitionOptions}</select></label><label>Original / cost round<select data-keeper-v13-round="${team}:${slot}">${roundOptions}</select></label><label>Salary<input data-keeper-v13-salary="${team}:${slot}" type="number" min="0" max="1000" step="1" value="${Number.isFinite(entry.salary) ? entry.salary : ""}" placeholder="Auction only" /></label><label>Years already kept<input data-keeper-v13-years="${team}:${slot}" type="number" min="0" max="10" value="${entry.yearsKept || 0}" /></label><label class="keeper-franchise-toggle"><input data-keeper-v13-franchise="${team}:${slot}" type="checkbox" ${entry.franchise ? "checked" : ""} ${franchiseDisabled}/> Franchise exemption</label><span>${entry.playerId ? escapeHtml(keeperStatusForTeam(team, entry)) : "Open keeper slot"}</span></div>`;
    }).join("");
    return `<div class="keeper-team-editor"><strong>${escapeHtml(names[index] || `Team ${team}`)}</strong>${rows}</div>`;
  }).join("");
};
...
function updateV13KeeperEntry(key, field, rawValue) {
  ...
  if (field === "player") {
    ...
    entries[slot] = { ...entries[slot], round: round >= 1 && round <= LEAGUE.rounds ? round : "" };
...
function valuePercentilesBySource(prepared) {
  ...
  sourceRows.forEach((rows, source) => {
    ...
    ["QB", "RB", "WR", "TE", "K", "DEF"].forEach((position) => {
      ...
      replacementByPosition[position] = positionRows[Math.min(positionRows.length - 1, boundary - 1)]?.adjustedWeekly ?? positionRows[positionRows.length - 1]?.adjustedWeekly ?? 0;
    });
    const valued = rows.map((row) => ({ ...row, vor: row.adjustedWeekly - (replacementByPosition[row.player.position] || 0) })).sort((a, b) => b.vor - a.vor);
    valued.forEach((row, index) => result.set(`${row.player.id}|${source}`, { source, adjustedWeekly: row.adjustedWeekly, replacementValue: replacementByPosition[row.player.position] || 0, vor: row.vor, percentile: valued.length > 1 ? 1 - index / (valued.length - 1) : 0.5 }));
  });
  return result;
}
...
rebuildConsensusPlayers = function rebuildConsensusPlayersV13(importedRows = []) {
  ...
  if (state.seedRankingsEnabled) BASE_PLAYERS.forEach((player) => byId.set(player.id, createBase({ ...player, sourceRanks: { [SEED_SOURCE.name]: player.adp || player.rank }, importedAdps: [player.adp] })));
  importedRows.forEach((row) => {
  ...
  const sourceListSizes = new Map();
  byId.forEach((player) => Object.entries(player.sourceRanks || {}).forEach(([source, rank]) => { if (Number.isFinite(rank)) sourceListSizes.set(source, Math.max(sourceListSizes.get(source) || 0, Math.ceil(rank))); }));
  let prepared = [...byId.values()].map((player) => {
    ...
    return {
      ...
      importedProjections: projections, projectionSources: [...new Set(projections.map((item) => item.source))],
      weightedProjection: projections.length ? average(projections.map((item) => item.weeklyValue)) : null,
  ...
  prepared = prepared.map((player) => {
    const normalizedParts = player.sourceParts.map((part) => {
      ...
      return { ...part, valuePercentile: value?.percentile ?? null, adjustedProjection: value?.adjustedWeekly ?? null, replacementProjection: value?.replacementValue ?? null, vor: value?.vor ?? null, evidencePercentile: value ? part.percentile * 0.30 + value.percentile * 0.70 : part.percentile };
    });
    ...
    return {
      ...
      weightedProjection: projectionRows.length ? average(projectionRows.map((part) => part.adjustedProjection)) : player.weightedProjection,
    };
  }).sort((a, b) => b.baseScore - a.baseScore || a.adp - b.adp);
  prepared.forEach((player, index) => { player.baseConsensusRank = index + 1; });
  const positionCounters = {};
  ...
  const scored = prepared.map((player) => {
    ...
    const confidenceAnalysis = rankingConfidenceAnalysis(player);
    const labScore = player.baseScore * preset.base + leagueAnalysis.score * preset.league + guideAnalysis.score * preset.guide;
    return { ...player, leagueFitScore: leagueAnalysis.score, leagueAnalysis, guideSignalScore: guideAnalysis.score, guideAnalysis, confidenceAnalysis, labScore, overlayPreset: preset.label, labReasons: [...guideAnalysis.positive.map((item) => item.label), ...guideAnalysis.risks.map((item) => item.label)] };
  }).sort((a, b) => b.labScore - a.labScore || a.baseConsensusRank - b.baseConsensusRank);
  const ranked = scored.map((player, index) => ({ ...player, rank: index + 1, consensusRank: index + 1, customRank: index + 1, convictionRank: index + 1, modelEdge: player.baseConsensusRank - (index + 1), leagueAdjustment: player.leagueFitScore - 50, guideAdjustment: player.guideSignalScore - 50 }));
...
function draftPortfolioProfile() {
  ...
  if (DRAFT_PORTFOLIO_PROFILE_CACHE.key === key && DRAFT_PORTFOLIO_PROFILE_CACHE.value) return DRAFT_PORTFOLIO_PROFILE_CACHE.value;
...
function renderCalibrationCenter() {
  ...
  return `<section class="sim-section calibration-center"><div class="sim-section-heading"><div><p class="eyebrow">Simulator Calibration Center</p><h3>Prediction quality, not just simulation volume</h3></div><span>${calibration.evaluated || 0} evaluated picks</span></div><div class="calibration-grid"><article class="calibration-card"><strong>${Number.isFinite(calibration.positionAccuracy) ? `${Math.round(calibration.positionAccuracy * 100)}%` : "—"}</strong><span>Top-position accuracy</span></article><article class="calibration-card"><strong>${Number.isFinite(calibration.playerAccuracy) ? `${Math.round(calibration.playerAccuracy * 100)}%` : "—"}</strong><span>Exact-player accuracy</span></article><article class="calibration-card"><strong>${Number.isFinite(calibration.survivalMeanAbsoluteError) ? calibration.survivalMeanAbsoluteError.toFixed(3) : "—"}</strong><span>Survival probability error</span></article></div><div class="calibration-card">${buckets || "No survival forecasts have resolved yet."}</div><p class="sim-conclusion">${escapeHtml(calibration.conclusion || "Predictions will calibrate as live draft picks resolve.")}</p></section>`;
}
...
renderBulkSimulator = function renderBulkSimulatorV13() {
  ...
  if (!results || !state.bulk.results?.summary) return;
  results.insertAdjacentHTML("afterbegin", `${renderCalibrationCenter()}${renderRobustDecisionSummary()}`);
};
...
renderKeeperRankings = function renderKeeperRankingsV13() {
  ...
  $("keeperRankingsList").innerHTML = teams.map((team) => {
    const candidates = keeperRankingCandidatesForTeam(team).slice(0, 5);
    return `<section class="keeper-team-card" id="keeper-team-${team}"><div class="keeper-team-heading"><div><p class="eyebrow">Top keeper options</p><h3>${escapeHtml(teamName(team))}</h3></div><span>${candidates.length} matched options</span></div>${candidates.length ? candidates.map((item, index) => `<article class="keeper-ranking-row"><div class="keeper-rank-number">${index + 1}</div><div class="keeper-rank-player"><strong>${escapeHtml(item.player.name)}</strong><span>${escapeHtml(item.player.position)} · ${escapeHtml(item.band)}</span><p>${escapeHtml(keeperRankingReason(item))} Rebuilt league market: pick ${item.adjustedMarketPick.toFixed(1)}. Next season: ${item.eligibleNextSeason ? `${item.nextEdge.toFixed(1)}-pick directional edge` : "ineligible under max-year rule"}.</p></div><div class="keeper-rank-stat"><span>Current cost</span><b>${item.round ? `R${item.round}` : item.salary != null ? `$${item.salary}` : "Need salary"}</b></div><div class="keeper-rank-stat"><span>League market</span><b>${item.adjustedMarketPick.toFixed(1)}</b></div><div class="keeper-rank-stat"><span>Current edge</span><b>${item.edge.toFixed(1)}</b></div><div class="keeper-rank-stat"><span>Next edge</span><b>${item.eligibleNextSeason ? item.nextEdge.toFixed(1) : "—"}</b></div><div class="keeper-rank-actions"><span class="keeper-value-pill">${escapeHtml(item.band)}</span></div></article>`).join("") : `<p class="empty">No ranked keeper candidates found.</p>`}</section>`;
  }).join("");
};
...
startCandidateOutcomeRecommendations = function startCandidateOutcomeRecommendationsV13() {
  ...
  const runTrial = () => {
    if (candidateOutcomeModelKey() !== modelKey) { finish(); return; }
    try {
      const item = queue[queueIndex];
      trialsByCandidate.get(item.candidate.id).push(simulateCandidateTrial(item.candidate, item.trial, modelKey));
      queueIndex += 1;
