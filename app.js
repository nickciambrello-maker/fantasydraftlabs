...
function pickValueBase(pickNumber) {
  ...
  const earlyCurve = 1320 / Math.pow(pickNumber + 7, 0.7);
  ...
  const lateFloor = Math.max(1.5, 8 * (1 - pickNumber / (totalPicks + 12)));
  return Math.max(lateFloor, earlyCurve * starterPremium * depthPremium * positionPremium);
}
...
function teamPickContextMultiplier(team, pickNumber) {
  ...
  if (currentValue < leagueAverage) multiplier += Math.min(0.12, (leagueAverage - currentValue) / leagueAverage * 0.16);
  if (roundOwned === 0) multiplier += 0.05;
  if (roundOwned >= 2) multiplier -= 0.035;
  if (gap >= LEAGUE.teams * 1.5) multiplier += 0.04;
...
function personaPick(team, pickNumber) {
  ...
  const candidates = availablePlayers().slice(0, personaCandidateLimit(persona));
...
function renderBoard() {
  ...
  for (let pick = 1; pick <= total; pick += 1) {
    ...
    const ownerName = viewedDraft ? draftTeamName(viewedDraft, order.team) : teamName(order.team);
    cells.push(`
      <div class="pick ${order.team === userTeam ? "user" : ""} ${!viewedDraft && pick === state.currentPick ? "active" : ""}">
        <div class="pick-number">${order.label} - ${escapeHtml(ownerName)}</div>
  ...
  $("draftBoard").style.gridTemplateColumns = `repeat(${league.teams}, minmax(112px, 1fr))`;
...
function renderRoster() {
  ...
  const counts = positionCounts(roster);
  const rows = ["QB", "RB", "WR", "TE", "K", "DEF"].map((pos) => {
    const names = rosterPicks
      .filter((pick) => pick.player.position === pos)
      .map((pick) => `${pick.player.name}${pick.keeper ? " (Keeper)" : ""}`)
      .join(", ") || "Open";
    return `<div class="roster-row"><div class="pos">${pos}</div><div>${names}</div></div>`;
  });
...
function renderAvailable() {
  ...
  const players = availablePlayers()
    ...
    .slice(0, 80);
  $("availableList").innerHTML = players
    .map((p) => `
      <div class="available-player" data-draft="${p.id}">
        <div>
          <div class="player-name">${p.name}</div>
          <div class="meta">${formatPlayer(p)}</div>
          <div class="mini-context">${escapeHtml(`${p.depthChartRole || ""}${p.depthChartRank ? ` - ${p.position}${p.depthChartRank}` : ""}`)}</div>
        </div>
        <span class="badge">#${Math.round(p.consensusRank)}</span>
      </div>
    `)
    .join("");
}
...
function tradePackageDetails(pickNumbers, receivingTeam, keeperAsset) {
  ...
  const keeperValue = keeperAsset?.surplus || 0;
  return {
    picks,
    pickValue,
    keeperAsset,
...
function tradePackageRows(packageDetails) {
  ...
  const keeperRow = keeper
    ? `<li>${escapeHtml(keeper.player.name)} ${keeper.player.position} keeper in Round ${keeper.round}: ${formatTradeValue(keeper.surplus)} surplus <small>Market pick ${Math.round(keeper.marketPick)}, cost around pick ${Math.round(keeper.costPick)}</small></li>`
    : "";
  ...
  $("tradeResults").innerHTML = `
    ...
    <div class="trade-results-grid">
      ...
      <section class="trade-method">
        <h3>Method</h3>
        <p>Pick values use a curved draft-capital model: early picks are worth disproportionately more, starter-window picks receive a league-settings premium, deep benches raise later pick value, and position demand adjusts the expected player at each pick.</p>
        <p>Team-specific value adjusts for whether a team is short on total draft capital, missing a pick in that round, has multiple picks in that round, or is filling a large gap between selections.</p>
        <p>Keeper assets are valued as market-pick value minus keeper-cost pick value, with small scoring and positional scarcity adjustments.</p>
      </section>
    </div>
  `;
}
