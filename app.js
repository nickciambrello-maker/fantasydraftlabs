...
function normalizeImportedRow(row, fallbackSource) {
  ...
  return {
    ...
    position: position ? normalizePosition(position) : "",
...
function rebuildConsensusPlayers(importedRows = []) {
  ...
  PLAYERS = enrichPlayerContext([...byId.values()].map((player) => {
    ...
    const bye = median(player.importedByes) || player.bye || null;
...
function projectionForPlayer(player) {
  ...
  const projection = Math.max(4, base - positionalIndex * decay + scoringProjectionBonus(player));
...
function simulatedSeasonAwards(analyses, picks, seasonCount = 500) {
  const random = seededAwardRandom(draftAwardSeed(picks));
  const league = activeLeague();
  const playoffTeams = Math.min(6, Math.max(2, Math.ceil(league.teams / 2)));
  const teams = analyses.map((analysis) => ({
    team: analysis.team,
    weeklyMean: (Number.isFinite(analysis.weeklyProjection) ? analysis.weeklyProjection : 100)
      + Math.max(-12, Math.min(12, analysis.value || 0)) * 0.12
      + Math.max(-12, Math.min(12, analysis.balance || 0)) * 0.04,
    playoffMean: (Number.isFinite(analysis.weeklyProjection) ? analysis.weeklyProjection : 100)
      + Math.max(-12, Math.min(12, analysis.value || 0)) * 0.08
      + Math.max(-12, Math.min(12, analysis.balance || 0)) * 0.03,
    weekStdDev: 15 + Math.max(0, 6 - (analysis.balance || 0)) * 0.3,
    championships: 0,
    lastPlaces: 0,
  }));
  const teamScore = (team, playoff = false) => {
    const deviation = team.weekStdDev * (playoff ? 1.25 : 1);
    return (playoff ? team.playoffMean : team.weeklyMean)
      + seededNormal(random) * deviation
      + (random() - 0.5) * 4;
  };
  ...
  for (let season = 0; season < seasonCount; season += 1) {
    ...
    while (bracket.length > 1) {
      ...
      for (let index = 0; index < Math.ceil(bracket.length / 2); index += 1) {
        ...
        const favoriteScore = teamScore(favorite, true);
        const underdogScore = teamScore(underdog, true);
        winners.push(favoriteScore >= underdogScore ? favorite : underdog);
      }
      bracket = winners;
    }
    const champion = bracket[0] || [...rows].sort((a, b) => b.wins - a.wins || b.points - a.points)[0];
    const last = [...rows].sort((a, b) => a.wins - b.wins || a.points - b.points)[0];
...
function allTeamAnalyses() {
  const picks = activePicks();
  return rankTeamAnalysisRows(
    Array.from({ length: activeLeague().teams }, (_, index) => analyzeTeam(index + 1, picks)),
    activeLeague()
  );
}
...
function currentPickAdvice() {
  ...
  const bestAnalysis = playerDecisionAnalysis(recs[0], state.userTeam, state.currentPick)
    .map((item) => `${item.label}: ${item.text}`)
    .join("\n");
  return `My top options at ${pickLabel(state.currentPick)}:\n${rows}\n\nBest lean: ${recs[0].name}.\n${bestAnalysis}\n\nRoster context: ${rosterNeedLabel(roster)}.`;
}
...
function playerDecisionAnalysis(player, team = state.userTeam, pickNumber = state.currentPick) {
  ...
  const rankText = `board rank #${Math.round(player.consensusRank || 999)}`;
  const projectionText = `${projectionForPlayer(player).toFixed(1)} projected avg pts`;
  const sourceText = player.sourceCount > 1 ? ` across ${player.sourceCount} sources` : "";
  const valueText = Number.isFinite(player.adp)
    ? adpGap >= 8
      ? `and he is ${adpGap.toFixed(1)} picks past ADP`
      : adpGap <= -12
        ? `even though this is ${Math.abs(adpGap).toFixed(1)} picks before ADP`
        : "and the price is close to market"
    : "with no reliable ADP anchor";
  const altScoreGap = bestAlternative
    ? Math.max(0, selected.score - recommendationScore(bestAlternative, team, pickNumber))
    : 0;
  ...
  return [
    {
      ...
      text: `${player.name} brings ${rankText}, ${projectionText}${sourceText}, ${player.depthChartRole ? ` and a ${player.depthChartRole.toLowerCase()} role` : ` ${valueText}`}.`
    },
    {
      label: "Why now",
    ...
    },
    {
      ...
      text: bestAlternative
        ? `He grades ${altScoreGap.toFixed(1)} points ahead of the next best option, ${bestAlternative.name}. ${samePositionAlternative ? `The nearest ${player.position} fallback is ${samePositionAlternative.name}.` : ""}`
        : "He is the clear top option left in this recommendation set."
    },
    {
      label: "Why this position",
      text: otherPositionAlternative
        ? otherPositionText
        : `${positionNeedText(player, roster)}, and the board does not show a better cross-position tradeoff at this slot.`
    },
  ];
}
...
function sleeperCandidateScore(player, team = state.userTeam, pickNumber = state.currentPick) {
  const baseScore = recommendationScore(player, team, pickNumber);
  const marketDelay = Math.max(0, (player.adp || player.consensusRank || 120) - pickNumber);
  return baseScore + Math.min(35, marketDelay * 0.18);
}

function isSleeperCandidate(player, pickNumber = state.currentPick) {
  const laterMarket = Number.isFinite(player.adp)
    ? player.adp >= pickNumber + 24 || player.adp >= 85
    : player.consensusRank >= 85;
  const upsideText = `${player.aiAnalysis || ""} ${player.sourceSummary || ""} ${player.upsideNote || ""}`;
  return laterMarket && (
    isYoungUpsidePlayer(player)
    || player.keeperValue > 0
    || /upside|ambiguous|competition|breakout|role/i.test(upsideText)
  );
}
...
function sleeperFlagReason(player, pickNumber) {
  const reasons = [];
  const marketPick = Number.isFinite(player.adp) ? player.adp : player.consensusRank;
  if (Number.isFinite(marketPick)) {
    if (marketPick >= pickNumber + 24) reasons.push(`market is still ${Math.round(marketPick - pickNumber)} picks away`);
    if (marketPick >= 85) reasons.push("late enough to stash without paying a premium");
  }
  if (isYoungUpsidePlayer(player)) reasons.push("young/upside profile");
  if (player.keeperValue > 0) reasons.push("keeper-value signal");
  if (player.upsideNote) reasons.push(player.upsideNote);
  if (player.depthChartRole) reasons.push(player.depthChartRole);
  if (/ambiguous|competition|breakout|role/i.test(`${player.aiAnalysis || ""} ${player.sourceSummary || ""}`)) reasons.push("role or breakout path worth monitoring");
  return reasons.slice(0, 3).join("; ") || playerShortReason(player, pickNumber);
}
...
function sleeperFlagSuggestionsAdvice() {
  ...
  const rows = suggestions
    ...
    .join("\n");
  return `Potential sleepers I would consider flagging at ${pickLabel(pickNumber)}:\n${rows}\n\nThese are watch-list suggestions, not automatic pick-now recommendations. I am looking for players far enough down market boards to be cheap, but with upside, keeper value, role uncertainty, or breakout paths that make them worth tracking.`;
}

function likelyAvailableAtPick(player, pickNumber) {
  const marketPick = Number.isFinite(player.adp) ? player.adp : player.consensusRank;
  if (!Number.isFinite(marketPick)) return true;
  return marketPick >= pickNumber - 10;
}
...
function playerScheduleRows(player) {
  ...
  if (nextPick && !state.draftedIds.has(player.id)) {
    rows.push({ label: "Draft room timing", text: `${player.name} is ${likelyAvailableAtPick(player, nextPick) ? "reasonably possible" : "less likely"} to reach your next pick at ${pickLabel(nextPick)}.` });
  }
  if (!rows.length) {
  ...
  modal.innerHTML = `
    ...
    <section class="player-detail-card" role="dialog" aria-modal="true" aria-label="${escapeHtml(player.name)} overview">
      ...
      <div class="player-detail-metrics">
        ...
        <div><strong>${projectionForPlayer(player).toFixed(1)}</strong><span>Proj</span></div>
...
function simulateBulkDraft(strategy, runIndex) {
  ...
  const seedValue = Number(seed.toFixed(5));
  ...
  try {
    ...
    state.mockSeed = seedValue;
    state.teamPersonas = bulkPersonaMix(runIndex, seedValue);
    ...
    const analyses = allTeamAnalysesForBulk(state.picks);
    const rankedUserAnalysis = analyses.find((analysis) => analysis.team === state.userTeam) || analyzeTeamForBulkRank(state.userTeam, state.picks);
    const detailedUserAnalysis = analyzeTeam(state.userTeam, state.picks);
    const userAnalysis = {
      ...detailedUserAnalysis,
      rank: rankedUserAnalysis.rank,
      grade: rankedUserAnalysis.grade,
      playoffOdds: rankedUserAnalysis.playoffOdds,
    ...
    return {
      ...
      allPicks: [...state.picks].sort((a, b) => a.pick - b.pick).map(compactPick),
...
function summarizeRunGroup(runs, label) {
  ...
  return {
    ...
    avgRank: average(runs.map((run) => run.rank)),
...
function summarizeBulkResults(runs) {
  ...
  const bestLeagueRuns = [...runs]
    .sort((a, b) => a.rank - b.rank || b.score - a.score || b.weeklyProjection - a.weeklyProjection)
    .slice(0, 8);
  return {
    createdAt: new Date().toISOString(),
...
function bulkExportManifest(results) {
  return {
    schemaVersion: 1,
    exportType: "fantasy-draft-lab-bulk-simulations",
    createdAt: new Date().toISOString(),
    appVersion: "v65",
    totalRuns: results.runs.length,
    controls: {
      requestedCount: state.bulk.count,
    ...
    files: [
      ...
      "summary/strategy-summary.csv",
      "summary/build-summary.csv",
      "runs/run-index.csv",
      "runs/run-001.json and one JSON file per simulation run",
      "picks/all-picks.csv",
      "availability/user-pick-availability.csv",
      "training/training-runs.jsonl",
    ],
  };
}
...
function bulkStrategySummaryCsv(summary) {
  const rows = summary.strategies.map((group) => ({
    ...
    runs: group.count,
    avgProjection: formatNumber(group.avgProjection),
    avgScore: formatNumber(group.avgScore),
    avgRank: formatNumber(group.avgRank),
    winRate: percent(group.winRate),
    top3Rate: percent(group.top3Rate),
    playoffOdds: formatNumber(group.avgPlayoffOdds, 0),
    bestRunId: group.best?.id || "",
    medianRunId: group.median?.id || "",
    worstRunId: group.worst?.id || "",
  }));
  return csvFromRows(["strategy", "runs", "avgProjection", "avgScore", "avgRank", "winRate", "top3Rate", "playoffOdds", "bestRunId", "medianRunId", "worstRunId"], rows);
}

function bulkBuildSummaryCsv(summary) {
  const rows = summary.builds.map((group) => ({
    firstFiveBuild: group.label,
    runs: group.count,
    avgProjection: formatNumber(group.avgProjection),
    avgScore: formatNumber(group.avgScore),
    avgRank: formatNumber(group.avgRank),
  ...
  return csvFromRows(["firstFiveBuild", "runs", "avgProjection", "avgScore", "avgRank", "winRate", "top3Rate", "bestRunId"], rows);
}
...
function bulkRunIndexCsv(runs) {
  const rows = runs.map((run) => ({
    ...
    rank: run.rank,
    grade: run.grade,
    playoffOdds: run.playoffOdds,
    weeklyProjection: formatNumber(run.weeklyProjection),
    score: formatNumber(run.score),
    value: formatNumber(run.value),
    firstFiveBuild: run.firstFiveBuild,
    firstFivePlayers: run.firstFivePlayers.join(" | "),
    userPicks: run.userPicks.map((pick) => `${pick.label} ${pick.player.name}`).join(" | "),
  }));
  return csvFromRows(["runIndex", "runId", "seed", "strategy", "strategyLabel", "rank", "grade", "playoffOdds", "weeklyProjection", "score", "value", "firstFiveBuild", "firstFivePlayers", "userPicks"], rows);
}

function bulkAllPicksCsv(runs) {
  const rows = runs.flatMap((run) => run.allPicks.map((pick) => ({
    runIndex: run.runIndex,
    runId: run.id,
    seed: run.seed,
    strategy: run.strategy,
...
function exportBulkSimulationsZip() {
  ...
  const files = [
    ...
    { path: "summary/summary.json", content: JSON.stringify(results.summary, null, 2) },
    { path: "summary/strategy-summary.csv", content: bulkStrategySummaryCsv(results.summary) },
    { path: "summary/build-summary.csv", content: bulkBuildSummaryCsv(results.summary) },
    ...
    ...results.runs.map((run) => ({
      path: `runs/run-${String(run.runIndex).padStart(3, "0")}.json`,
...
function bulkStrategySchedule() {
  const count = Math.max(1, Math.min(250, Number(state.bulk.count) || 50));
...
function updateBulkProgress() {
  ...
  if ($("bulkExportStatus")) {
    $("bulkExportStatus").textContent = state.bulk.exportStatus || (
      state.bulk.results?.runs?.length
        ? "Export creates a zip with summary CSVs, per-run JSON, pick-level data, availability snapshots, and a JSONL training file."
        : ""
    );
  }
}
...
function startBulkSimulations() {
  ...
  state.bulk.count = Math.max(1, Math.min(250, Number($("bulkCountInput").value) || 50));
  ...
  const runBatch = () => {
    const batchSize = state.bulk.count >= 100 || (state.bulk.mode === "single" && state.bulk.strategy === "weeklyEdge") ? 1 : 6;
    ...
    state.bulk.exportStatus = "Simulation batch ready. Export zip is available.";
    render();
  };
...
function renderAvailable() {
  ...
  const players = availablePlayers()
    .filter((p) => {
      if (state.positionFilter === "ALL") return true;
      if (state.positionFilter === "FLAGGED") return state.flaggedPlayerIds.has(p.id);
      return p.position === state.positionFilter;
    })
    ...
    .sort((a, b) => b.score - a.score);
  const actionBar = state.positionFilter === "FLAGGED"
    ? `
      <div class="available-action-bar">
        <div>
          <strong>Flagged players</strong>
          <span>${players.length ? `${players.length} available match${players.length === 1 ? "" : "es"}` : "No available flagged players match this view"}</span>
        </div>
        <div class="available-action-buttons">
          <button type="button" data-flagged-sleepers>Recommend flagged sleepers</button>
          <button type="button" data-sleeper-flag-suggestions>Who should I flag?</button>
        </div>
      </div>
    `
    : "";
  const emptyState = players.length
    ? ""
    : `<p class="available-empty">No players match this big-board view.</p>`;
  $("availableList").innerHTML = `
    ...
    .map(({ player: p }) => `
      <div class="available-player">
        <div>
          <button class="player-name player-name-button" type="button" data-player-detail="${p.id}">${escapeHtml(p.name)}</button>
          <div class="mini-context">${escapeHtml(p.depthChartRole || p.aiAnalysis || "Ranking profile")}</div>
        </div>
        <button class="flag-player ${state.flaggedPlayerIds.has(p.id) ? "active" : ""}" data-flag-player="${p.id}" type="button" aria-label="${state.flaggedPlayerIds.has(p.id) ? "Unflag" : "Flag"} ${escapeHtml(p.name)}">${state.flaggedPlayerIds.has(p.id) ? "On" : "Flag"}</button>
        ...
        <button class="draft-player-button" type="button" data-draft="${p.id}">Draft</button>
      </div>
    `)
    .join("")}
    ${emptyState}
...
function bulkReplayBoard(run) {
  if (!run?.allPicks?.length) {
    return `<p class="empty">No replay board is stored for this run. Run a new simulation batch to capture full replay data.</p>`;
  }
  ...
  return `
    <div class="bulk-replay-meta">
      ...
      <div><span>Picks</span><strong>${run.allPicks.length}</strong></div>
    </div>
...
function renderBulkSimulator() {
  ...
  const strategyRows = summary.strategies.map((group, index) => `
    <div class="bulk-table-row ${index === 0 ? "leader" : ""}">
      ...
      <span>#${formatNumber(group.avgRank, 1)}</span>
      <span>${percent(group.winRate)}</span>
  ...
  const bestLeagueRows = (summary.bestLeagueRuns || []).map((run) => `
    <button class="bulk-example-card ${state.bulk.selectedRunId === run.id ? "active" : ""}" data-bulk-run="${run.id}" type="button">
      <span>#${run.rank} in room - seed ${run.seed ?? "N/A"}</span>
      <strong>${escapeHtml(run.strategyLabel)} - ${escapeHtml(run.firstFiveBuild)}</strong>
      <small>${formatNumber(run.weeklyProjection)} starter pts - ${run.playoffOdds}% playoffs - ${escapeHtml(run.firstFivePlayers.slice(0, 3).join(", "))}</small>
    </button>
  `).join("");
  ...
  const runExplorer = bulkRunExplorer(runs, selected?.id);
  const replayBoard = bulkReplayBoard(selected);
  ...
  $("bulkResults").innerHTML = `
    <div class="bulk-hero">
      ...
      <div><strong>${percent(summary.winRate)}</strong><span>Room win rate across all runs</span></div>
      <div><strong>${selected?.seed ?? "N/A"}</strong><span>Selected run seed</span></div>
    </div>
    <div class="bulk-grid">
      <section>
        <h3>Strategy Comparison</h3>
        <div class="bulk-table">
          <div class="bulk-table-head"><span>Strategy</span><span>Runs</span><span>Starter</span><span>Score</span><span>Avg Rank</span><span>Wins</span><span>Top 3</span><span>Playoffs</span></div>
          ${strategyRows}
        </div>
      </section>
      <section>
        <h3>Best Against The League</h3>
        <div class="bulk-example-list">${bestLeagueRows || `<p class="empty">No winning runs yet.</p>`}</div>
      </section>
      <section>
      ...
      <section class="bulk-wide">
        <h3>Run Explorer</h3>
        <div class="bulk-run-explorer">${runExplorer}</div>
      </section>
      <section class="bulk-wide">
...
function renderCheatSheetSources() {
  ...
  const options = [
    ...
    `<option value="SEED">Built-in seed</option>`,
...
function renderCheatSheet() {
  ...
  $("cheatSheetSummary").textContent = `${players.length} players${players.length > shown.length ? `, showing top ${shown.length}` : ""} - ${availableCount} available`;
  const empty = `<p class="available-empty">No players match this cheat sheet view.</p>`;
  $("cheatSheetList").innerHTML = `
    ...
    const sources = player.sourceNames?.length ? player.sourceNames.join(" + ") : "No source";
    const role = player.sourceSummary || player.depthChartRole || player.aiAnalysis || "Ranking profile";
    return `
      <div class="cheat-sheet-row ${drafted ? "drafted" : ""}">
        <strong>#${Math.round(player.consensusRank || 999)}</strong>
        <div>
          <button class="player-name player-name-button" type="button" data-player-detail="${player.id}">${escapeHtml(player.name)}</button>
          <small>${drafted ? "Drafted" : "Available"}${player.keeperValue ? ` - keeper value ${player.keeperValue.toFixed(1)}` : ""}</small>
        </div>
        ...
        <span>${projectionForPlayer(player).toFixed(1)}</span>
        <span>${Number.isFinite(player.bye) ? player.bye : "-"}</span>
        <span title="${escapeHtml(sources)}">${player.sourceCount || 0}</span>
        <p>${escapeHtml(role)}</p>
      </div>
    `;
  }).join("") : empty}
...
async function importSelectedSleeperLeague() {
  ...
  try {
    ...
    const sourceText = importData.usedPreviousLeagueForKeepers
      ...
      : "";
    state.sleeper.status = `Imported ${importData.leagueName}: ${importData.teams.length} teams, ${candidateCount} keeper candidates, draft order from ${draftSlotSource}, and ${appliedTradedPickCount}/${tradedPickCount} Sleeper traded-pick records applied to Pick Order (${leagueTradedPickCount} league, ${draftTradedPickCount} draft).${sourceText}`;
  } catch (error) {
...
function setupControls() {
  ...
  $("bulkCountInput").addEventListener("change", (event) => {
    state.bulk.count = Math.max(1, Math.min(250, Number(event.target.value) || 50));
