...
function normalizeImportedRow(row, fallbackSource) {
  const rawName = importedValue(row, ["name", "player", "player name", "player_name", "full name", "full_name", "player bye", "player team bye", "player (bye)"]);
  const parsedName = parseFantasyProsPlayerCell(rawName);
  const name = parsedName.name;
  const position = importedValue(row, ["position", "pos"]);
  const team = importedValue(row, ["team", "tm", "nfl team", "nfl_team", "nflteam"]) || parsedName.team;
  const source = importedValue(row, ["source"]) || fallbackSource;
  const rank = numberValue(importedValue(row, ["rank", "rk", "ecr", "overall", "overall rank", "value rank", "value_rank"]));
  const adp = numberValue(importedValue(row, ["adp", "avg", "avg adp", "average", "average adp", "average pick", "avg pick", "avg_pick", "avgpick"]));
  const projection = numberValue(importedValue(row, ["projection", "points", "fantasy points", "fantasy_points", "fpts"]));
  const tier = numberValue(importedValue(row, ["tier", "tiers"]));
...
function csvHeaderScore(row) {
  ...
  const hasPlayer = keys.some((key) => ["player", "playername", "name", "fullname", "playerbye", "playerteambye"].includes(key));
  const hasRank = keys.some((key) => ["rank", "rk", "ecr", "overall", "overallrank", "adp", "avg", "average", "avgadp", "averageadp", "projection", "points", "fpts"].includes(key));
  return Number(hasPlayer) + Number(hasRank);
}

function parseCsv(text) {
  const rows = parseCsvRows(text).filter((row) => row.some(Boolean));
  const headerIndex = rows.findIndex((row) => csvHeaderScore(row) >= 2);
  const headers = (headerIndex >= 0 ? rows[headerIndex] : rows[0] || []).map((header) => header.trim());
  return rows
...
async function importRankingsFile(file) {
  ...
  if (!rows.length) {
    return {
      ok: false,
      message: `${file.name}: no usable rankings found. Try FantasyPros-style columns like Player (Bye), PLAYER NAME, Rank/RK, AVG, ADP, projection, or points.`,
    };
  }
...
function applySleeperTradedPicksToRoundOrders(tradedPicks, rosters, season, draft) {
  const orders = sleeperBaseRoundOrders(rosters, draft);
...
async function importSelectedSleeperLeague() {
  ...
  try {
    ...
    const sourceText = importData.usedPreviousLeagueForKeepers
      ...
      : "";
    state.sleeper.status = `Imported ${importData.leagueName}: ${importData.teams.length} teams, ${candidateCount} keeper candidates, draft order from ${draftSlotSource}, and ${tradedPickCount} Sleeper traded-pick records.${sourceText}`;
  } catch (error) {
