...
function tradePackageCandidates(team, receivingTeam, includeKeepers = false, target = null) {
  ...
  if (forcedKeeper) {
    candidates.push({
      picks: [],
      keeper: forcedKeeper,
      value: keeperTradeAssetValue(forcedKeeper),
    });
    owned
...
function generateTradeIdeas() {
  ...
  pairs.forEach(([teamA, teamB]) => {
    ...
    packagesA.forEach((aPackage) => {
      packagesB.forEach((bPackage) => {
        ...
        const ideaScore = valueMatch + strategicScore + targetFit.score - Math.abs(100 - valueMatch) * 0.2;
        ideas.push({
  ...
  if (target.targetPlayer) {
    const targeted = sortedIdeas.filter((idea) => ideaHasKeeperPlayer(idea, target.targetPlayer));
    if (targeted.length) sortedIdeas = [...targeted, ...sortedIdeas.filter((idea) => !ideaHasKeeperPlayer(idea, target.targetPlayer))];
  }
...
async function importRankingsFile(file) {
  ...
  if (!rows.length) {
    $("importStatus").textContent = "No usable rankings found. Try a CSV with FantasyPros-style columns like Player (Bye), PLAYER NAME, Rank/RK, AVG, ADP, projection, or points.";
    return;
  }
