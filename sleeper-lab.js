(function installSleeperLab() {
  ...
  if (!globalThis.FDLSleeperModel || typeof globalThis.FDLSleeperModel.buildSleeperProfile !== "function") {
    console.error("Sleeper Lab model is unavailable.");
    return;
  }
  ...
  function saveSleeperLabState() {
    try {
      ...
      state.sleeperLab.status = "Sleeper Lab changes are active, but this browser blocked local saving.";
    }
  }
  ...
  function sleeperCardHtml(player) {
    ...
    return `<article class="sleeper-card ${profile.isSleeper ? "qualifies" : "does-not-qualify"}" data-sleeper-card="${escapeHtml(player.id)}">
      <div class="sleeper-card-head"><div><span class="sleeper-score" aria-label="Sleeper Score ${Math.round(profile.sleeperScore)}"><b>${Math.round(profile.sleeperScore)}</b><small>Sleeper Score</small></span><div><button class="player-name-button" type="button" data-sleeper-player="${escapeHtml(player.id)}"><strong>${escapeHtml(player.name)}</strong></button><small>${escapeHtml(player.position)} · ${escapeHtml(player.team || "FA")} · Lab #${Math.round(player.consensusRank || 999)}</small></div></div><span class="confidence-badge confidence-${profile.confidenceLabel.toLowerCase()}">${escapeHtml(profile.confidenceLabel)}</span></div>
      ...
      <div class="sleeper-three-questions"><p><strong>Why sleeper</strong>${escapeHtml(profileEvidenceText(profile, "positive"))}</p><p><strong>Why not</strong>${escapeHtml(profile.failureReasons.join(" ") || profileEvidenceText(profile, "negative"))}</p><p><strong>What must happen</strong>${escapeHtml(profile.catalystDescription)}</p></div>
  ...
  function calibrationHtml() {
    ...
    if (!outcomes.length || !savedProfiles.length) {
      return `<p><strong>Calibration unavailable.</strong> Import season outcomes and save at least one completed draft containing draft-time Sleeper Scores. No historical outcome is fabricated.</p>`;
    }
  ...
  function renderSleeperLab() {
    ...
    $("sleeperLabBoard").innerHTML = players.slice(0, 120).map(sleeperCardHtml).join("") || `<p class="empty">No players match the current Sleeper Lab filters.</p>`;
  ...
  renderAvailable = function sleeperAwareRenderAvailable() {
    ...
    document.querySelectorAll("#availableList .available-player").forEach((row) => {
      ...
      if (playerCell) playerCell.insertAdjacentHTML("beforeend", `<span class="inline-sleeper-score" title="${escapeHtml(profile.archetype)} · ${escapeHtml(profile.confidenceLabel)} confidence">Sleeper ${Math.round(profile.sleeperScore)}</span>`);
    });
    return result;
  };
  ...
  renderRecommendations = function sleeperAwareRenderRecommendations() {
    ...
    document.querySelectorAll("#decisionCenter [data-player-detail]").forEach((button) => {
      ...
      const p = profileFor(player);
      container.insertAdjacentHTML("beforeend", `<p class="decision-sleeper-read"><strong>Sleeper ${Math.round(p.sleeperScore)}</strong> · ${escapeHtml(p.archetype)} · ${escapeHtml(p.confidenceLabel)} confidence${p.rosterRedundancyPenalty >= 20 ? " · Watch-list only due to redundancy" : ""}</p>`);
    });
    return result;
  };
  ...
  openPlayerDetail = function sleeperAwareOpenPlayerDetail(playerId) {
    ...
    if (player && grid && !grid.querySelector(".player-sleeper-profile")) {
      const p = profileFor(player);
      grid.insertAdjacentHTML("beforeend", `<section class="player-sleeper-profile"><h4>Sleeper Lab</h4><p><strong>${Math.round(p.sleeperScore)}/100 · ${escapeHtml(p.archetype)}</strong> · ${escapeHtml(p.confidenceLabel)} confidence</p><p><strong>Role:</strong> ${escapeHtml(p.currentRole)} → ${escapeHtml(p.ceilingRole)}.</p><p><strong>Catalyst:</strong> ${escapeHtml(p.catalystDescription)}</p><p><strong>Blocker:</strong> ${escapeHtml(p.primaryBlocker)}</p><p><strong>Window:</strong> Round ${p.targetRound}, latest safe pick ${p.latestSafePick}, survival ${p.survivalToNextPick === null ? "Unknown" : `${Math.round(p.survivalToNextPick * 100)}%`}.</p></section>`);
    }
    return result;
  };
  ...
  renderCheatSheet = function sleeperAwareRenderCheatSheet() {
    ...
    document.querySelectorAll("#cheatSheetList .cheat-sheet-row").forEach((row, index) => {
      ...
      const p = profileFor(player);
      cell.insertAdjacentHTML("beforeend", `<small class="inline-sleeper-score">Sleeper ${Math.round(p.sleeperScore)} · ${escapeHtml(p.archetype)}</small>`);
    });
    return result;
  };
  ...
  buildDraftPlanPriority = function sleeperAwareBuildDraftPlanPriority(summary, survivalRows) {
    ...
    rows.forEach((row) => {
      ...
      if (eligible && boost >= 2) row.tags = [...new Set([...(row.tags || []), "Sleeper Window"] )];
  ...
  assistantCompactContextSummary = function sleeperAwareAssistantCompactContextSummary() {
    ...
    return assistantSafeClone({ ...context, sleeperLab: { modelVersion: "sleeper-lab-v1", weights: SLEEPER_SCORE_WEIGHTS, topTargets: targets.targets, calibrationAvailable: Boolean(state.sleeperLab.outcomeRows.length) } });
  };
  ...
  function getSleeperTargetsTool(args = {}) {
    ...
    if (positions?.some((position) => !["QB", "RB", "WR", "TE"].includes(position)) || (positions?.length || 0) > 4) {
      throw new Error("Sleeper target positions must contain at most QB, RB, WR and TE.");
    }
    ...
    if (typeof args.targetType !== "string" || !allowedTypes.includes(args.targetType)) throw new Error("Unsupported sleeper target type.");
    ...
    return assistantSafeClone({ modelVersion: "sleeper-lab-v1", deterministic: true, targetType, sampleSize: targets.length, targets, limitations: targets.length ? [] : ["No available player matched every requested deterministic filter."] });
  }
  ...
  renderPostDraftProcessGrade = function sleeperAwareRenderPostDraftProcessGrade(grade) {
    ...
    const insert = `<section class="process-grade-section sleeper-post-grade"><div class="section-heading"><div><p class="eyebrow">Sleeper portfolio</p><h3>${portfolio.count} qualified sleeper${portfolio.count === 1 ? "" : "s"} drafted</h3></div><span>${portfolio.count ? `${portfolio.averageScore.toFixed(1)} average score` : "No forced sleeper quota"}</span></div>${portfolio.targets.length ? listItemsHtml(portfolio.targets) : `<p>No qualified sleeper was drafted. This is not automatically a process failure.</p>`}${portfolio.redundancyWarnings.length ? `<p><strong>Redundancy warnings:</strong> ${escapeHtml(portfolio.redundancyWarnings.join(", "))}</p>` : ""}</section>`;
    return `${html}${insert}`;
  };
  ...
  exportCustomBoardCsv = function sleeperAwareExportCustomBoardCsv() {
    ...
    state.sleeperLab.status = "Exported the deterministic Sleeper Lab board.";
    renderSleeperLab();
  };
