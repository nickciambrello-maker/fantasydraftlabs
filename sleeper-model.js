(function installSleeperModel(globalScope) {
  ...
  function buildSleeperProfile(player, context = {}) {
    ...
    const isSleeper = sleeperScore >= 58 && price.score >= 58 && nonPriceSignals >= 2 && !["K", "DEF"].includes(String(safePlayer.position || "").toUpperCase());
    ...
    pushEvidence(evidence, isSleeper, { type: "qualification", label: "Sleeper qualification", value: "Price edge plus at least two non-price strengths", direction: "positive", source: "Sleeper Lab rules" });
    pushEvidence(evidence, !isSleeper && finite(safePlayer.adp) !== null && safePlayer.adp >= 120, { type: "qualification", label: "Late ADP is insufficient", value: "The player does not meet the full structured sleeper threshold", direction: "negative", source: "Sleeper Lab rules" });
